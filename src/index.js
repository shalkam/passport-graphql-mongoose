...
module.exports = function(schema, options) {
  ...
  // To get a list of supported hashes use crypto.getHashes()
  options.passwordValidator = options.passwordValidator || (function(password, cb) {
      cb(null);
    });
  ...
  options.hashSaltField = options.hashSaltField || 'hashSalt';
  ...
  if (options.limitAttempts) {
    ...
    // 100 ms
    options.maxInterval = options.maxInterval || 300000;
    // 5 min
    options.maxAttempts = options.maxAttempts || Infinity;
  }

  options.errorMessages = options.errorMessages || {};
  options.errorMessages.MissingPasswordError = options.errorMessages.MissingPasswordError ||
    'No password was given';
  options.errorMessages.AttemptTooSoonError = options.errorMessages.AttemptTooSoonError ||
    'Account is currently locked. Try again later';
  options.errorMessages.TooManyAttemptsError = options.errorMessages.TooManyAttemptsError ||
    'Account locked due to too many failed login attempts';
  options.errorMessages.NoSaltValueStoredError = options.errorMessages.NoSaltValueStoredError ||
    'Authentication not possible. No salt value stored';
  options.errorMessages.IncorrectPasswordError = options.errorMessages.IncorrectPasswordError ||
    'Password or username are incorrect';
  options.errorMessages.IncorrectUsernameError = options.errorMessages.IncorrectUsernameError ||
    'Password or username are incorrect';
  options.errorMessages.MissingUsernameError = options.errorMessages.MissingUsernameError ||
    'No username was given';
  options.errorMessages.UserExistsError = options.errorMessages.UserExistsError ||
  ...
  schemaFields[options.hashSaltField] = { type: String, select: false };
  ...
  schema.methods.setPassword = function(password, cb) {
    if (!password) {
      cb(new errors.MissingPasswordError(options.errorMessages.MissingPasswordError));
    }
    ...
    options.passwordValidator(password, function(err) {
      ...
      hashSalt(password).hash(function(error, hash) {
        if (error) cb(new Error('Something went wrong!'));
        self.set(options.hashSaltField, hash);
        cb(null, self);
      });
    });
  };
  ...
  function authenticate(user, password) {
    return new Promise(function(resolve, reject) {
      if (options.limitAttempts) {
        var attemptsInterval = Math.pow(
          options.interval,
          Math.log(user.get(options.attemptsField) + 1)
        );
        var calculatedInterval = attemptsInterval < options.maxInterval
          ? attemptsInterval
          : options.maxInterval;
        ...
        if (Date.now() - user.get(options.lastLoginField) < calculatedInterval) {
          ...
          user.save();
          reject(new errors.AttemptTooSoonError(options.errorMessages.AttemptTooSoonError));
        }

        if (user.get(options.attemptsField) >= options.maxAttempts) {
          reject(new errors.TooManyAttemptsError(options.errorMessages.TooManyAttemptsError));
        }
      }
      if (!user.get(options.hashSaltField)) {
        reject(new errors.NoSaltValueStoredError(options.errorMessages.NoSaltValueStoredError));
      }
      hashSalt(password).verifyAgainst(user.get(options.hashSaltField), function(error, verified) {
          ...
          if (options.limitAttempts) {
            ...
            user.save(function(saveErr) {
              ...
              if (user.get(options.attemptsField) >= options.maxAttempts) {
                ...
                reject(new errors.IncorrectPasswordError(
                  options.errorMessages.IncorrectPasswordError
                ));
              }
            });
          } else {
            reject(new errors.IncorrectPasswordError(options.errorMessages.IncorrectPasswordError));
          }
        } else {
          if (options.limitAttempts) {
            user.set(options.lastLoginField, Date.now());
  ...
  schema.methods.authenticate = function(password) {
    ...
    if (!self.get(options.hashSaltField)) {
      return self.constructor
        .findByUsername(self.get(options.usernameField), true)
        .then(function(user) {
          if (user) {
          ...
          return new Promise(function(resolve, reject) {
            if (!user) {
              reject(new errors.IncorrectUsernameError(
      ...
      return authenticate(self, password);
    }
  };
  ...
  schema.statics.deserializeUser = function() {
    ...
    return function(username, cb) {
      self.findByUsername(username).then(function(user) {
        cb(null, user);
      });
    };
  };

  schema.statics.register = function(user, password) {
    ...
    if (!user.get(options.usernameField)) {
      return new Promise(function(resolve, reject) {
        reject(new errors.MissingUsernameError(options.errorMessages.MissingUsernameError));
      });
    }

    var self = this;
    return self
      .findByUsername(user.get(options.usernameField))
      .then(function(existingUser) {
        return new Promise(function(resolve, reject) {
          if (existingUser) {
            reject(new errors.UserExistsError(options.errorMessages.UserExistsError));
          } else {
  ...
  schema.statics.findByUsername = function(username, selectHashSaltFields) {
    // if specified, convert the username to lowercase
    ...
    if (selectHashSaltFields) {
      query.select('+' + options.hashSaltField);
    }
